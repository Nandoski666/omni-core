...
# --- CONFIGURACIÓN (Mete esto en tu .env) ---
# Le pongo "omni_pro_2026" como valor por defecto por si el .env falla
...
# --- MIDDLEWARE (CORS para tu Dashboard en Angular) ---
app.add_middleware(
    CORSMiddleware,
...
async def get_omni_response(user_text: str):
    """Llama a Groq para procesar el mensaje del cliente."""
    try:
        chat_completion = client.chat.completions.create(
            messages=[
                {"role": "system", "content": "Eres OMNI, el AI Setter experto de una agencia de automatización. Tu objetivo es calificar al cliente y agendar citas."},
...
async def send_whatsapp_message(to_phone: str, text: str):
    """Envía la respuesta de vuelta al cliente por la API de Meta."""
...
async def ask_omni(request: ChatRequest):
    """Para tu dashboard de Angular."""
    answer = await get_omni_response(request.prompt)
    return {"answer": answer}

@app.get("/webhook")
async def verify_webhook(request: Request):
    """Verificación inicial de Meta con Diagnóstico."""
    mode = request.query_params.get("hub.mode")
    token = request.query_params.get("hub.verify_token")
    challenge = request.query_params.get("hub.challenge")

    # Prints para cazar el error en tu terminal
    print(f"DEBUG - Token enviado por Meta: '{token}'")
    print(f"DEBUG - Token esperado por OMNI: '{WHATSAPP_VERIFY_TOKEN}'")

    if mode == "subscribe" and token == WHATSAPP_VERIFY_TOKEN:
        print("¡CONECTADOS, CUADRO! ✅")
        return int(challenge)
    
    print("¡FALLÓ LA CLAVE, MI LLAVE! ❌")
...
async def receive_whatsapp(request: Request, background_tasks: BackgroundTasks, x_hub_signature_256: str = Header(None)):
    ...
    # Validación de seguridad SHA-256
    ...
    try:
        if "messages" in data["entry"][0]["changes"][0]["value"]:
            ...
            if user_text:
                ...
                # Mandamos la tarea al fondo para responder 200 OK rápido
